...
class CreateBoardTrackingTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('board_tracking', function (Blueprint $table) {
            $table->increments('id');
...
//                ->on('board')->onUpdate('cascade')->onDelete('cascade');
        ...
        \Illuminate\Support\Facades\DB::unprepared('ALTER TABLE `board_tracking` DROP PRIMARY KEY, ADD PRIMARY KEY(id,board_id);');
        \Illuminate\Support\Facades\DB::unprepared('ALTER TABLE `board_tracking` PARTITION BY KEY (id,board_id) PARTITIONS 4;');
    }
    ...
    {
        Schema::dropIfExists('board_tracking');
    }
}
