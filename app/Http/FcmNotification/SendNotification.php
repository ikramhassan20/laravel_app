...
class SendNotification extends HTTPSender
{
     ...
     */
    public function sendTo($to, Options $options = null, PayloadNotificationDataBinding $notification = null, PayloadData $data = null, $server_key = null)
    {
        $response = null;
        if (is_array($to) && !empty($to)) {
            $partialTokens = array_chunk($to, self::MAX_TOKEN_PER_REQUEST, false);
            foreach ($partialTokens as $tokens) {
                $request = new NotificationRequest($tokens, $options, $notification, $data, null, $server_key);
            ...
            $request = new NotificationRequest($to, $options, $notification, $data, null, $server_key);
            $responseGuzzle = $this->post($request);
